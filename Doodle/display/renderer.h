...
namespace doodle {
	...
	public:
		...
		std::enable_if_t<std::is_same_v<SDL_Point, T>, void> 
			draw_points(const Container<T>& points) const noexcept;
		...
		template <template<class> class Container, class T>
		std::enable_if_t<std::is_same_v<std::pair<SDL_Point, SDL_Point>, T>, void>
			draw_lines(const Container<T>& lines) const noexcept;
		...
		template <template<class> class Container, class T>
		std::enable_if_t<std::is_same_v<SDL_Rect, T>, void>
			draw_rects(const Container<T>& rects) const noexcept;
		...
		template <template<class> class Container, class T>
		std::enable_if_t<std::is_same_v<SDL_Rect, T>, void> 
			draw_filled_rects(const Container<T>& filled_rects) const noexcept;
	...
	template<template<class> class Container, class T>
	inline std::enable_if_t<std::is_same_v<SDL_Point, T>, void> 
		Renderer::draw_points(const Container<T> & points) const noexcept
	{
	...
	template<template<class> class Container, class T>
	inline std::enable_if_t<std::is_same_v<std::pair<SDL_Point, SDL_Point>, T>, void> 
		Renderer::draw_lines(const Container<T> & lines) const noexcept
	{
		for (auto line : lines)
			draw_line(line);
	}
	...
	template <template<class> class Container, class T>
	inline std::enable_if_t<std::is_same_v<SDL_Rect, T>, void> 
		Renderer::draw_rects(const  Container<T> & rects) const noexcept
	{
		for (auto rect : rects)
			draw_rect(rect);
	}
	...
	template <template<class> class Container, class T>
	inline std::enable_if_t<std::is_same_v<SDL_Rect, T>, void>
		Renderer::draw_filled_rects(const Container<T> & filled_rects) const noexcept
	{
